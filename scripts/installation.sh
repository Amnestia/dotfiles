...
pacstrap -K /mnt base base-devel linux linux-firmware amd-ucode vim git e2fsprogs sudo dhcpcd pacman-contrib
...
hostnamectl set-hostname hostnameeee
...
# Setup mkinitcpio
sudo cp ./etc/initcpio/openswap.hooks /etc/initcpio/hooks/openswap
sudo cp ./etc/initcpio/openswap.install /etc/initcpio/install/openswap
...
# GRUB_CMDLINE_LINUX_DEFAULT="linux /vmlinuz-linux cryptdevice=UUID=uuidrootpartitionLUKS:root:allow-discards root=/dev/mapper/root resume=/dev/mapper/swap loglevel=3 quiet splash l1tf=full,force mds=full,nosmt mitigations=auto,nosmt nosmt=force"
